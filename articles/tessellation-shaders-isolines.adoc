...
_Patches_ are an important concept in tessellation shaders, before passing the vertices to a tessellation shader, the vertices must be split into patches and then the TCS must be told how many vertices are in each patch.
Teling the TCS how many vertices are in a patch is done with the function call:
