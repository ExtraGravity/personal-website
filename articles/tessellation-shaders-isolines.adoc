...
Putting this together, if the vertex shader received the points `a`, `b`, `c`, `d`, `e`, `f`, and our specified patch size was 3.
The Tessellation control shader would receive the 2 patches `[a, b, c]` and `[d, e, f]`.
