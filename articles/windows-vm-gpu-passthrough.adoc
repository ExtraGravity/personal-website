...
To get audio from the guest VM to the host, I opted for a more hardware oriented solution compared to a software solution, you can read the <<audio, audio section>> for more specifics.
...
With this new information, the kernel must load up differently.
...
With all these changes, you should be able to call `lsmod | grep vfio`, `dmesg | grep pci-stub`, `dmesg | grep VFIO`, and see something similar to the following.
...
For my Windows VM, I'll used the Windows 10 Anniversary ISO.
